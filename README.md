...
The project AUDIOS (Audio Deepfake Detection using Self-Supervised Learning) is a machine learning pipeline for audio dataset preprocessing, feature extraction, and model training for the detection of audios generated by AI.
...
5. **Model Testing**: Testing of the trained models.
...
In order know more about the dataset and download it, please follow the instructions in the [Dataset Preparation](DatasetPreparation.md) document. To train or test the models on the dataset, some additional steps are required, including processing the audio files, generating CSV files, and splitting the data. Detailed commands and instructions for these steps are outlined in the next section.
...
- **Test a model:**
  ...
  - `<run>` with the number of the best run (i.e. the run that resulted in the best model)
...
│   │   ├── test_mlp.py          # Script for testing MLP performance
...
├── notebooks/
│   ├── CNN_baseline_model.ipynb     # Notebook for CNN baseline training
...
│   ├── models.py                # Utilities for hyperparameter tuning
├── visualize/
...
├── prepare_dataset.sh           # Shell script for dataset preparation
...
Contains Jupyter Notebooks for exploratory and preparatory tasks like dataset exploration, normalization, and splitting. These mostly use the function in models and utils folders.

Also contains the main notebook for the YAMNet Baseline Model training and testing.
